...
class GenericClassifier:
    ...
    def initalize_containers(self, features, feat_select, dummy):
        ...
        self.n_features_orig = len(self.dataset.get_feature_names())
        ...
        self.odds_ratio = tools.mask_diagonal(np.ma.masked_array(np.zeros(self.comp_dims + (self.n_features, ))))
        self.thetas = tools.mask_diagonal(np.ma.masked_array(np.zeros(self.comp_dims + (2, self.n_features, ))))
    ...
    def classify(self, features=None, scoring='accuracy', X_threshold=None, feat_select=None, processes=1, class_weight='auto', dummy=None):
        ...
        try:
            ...
            for output in pool.imap(
                ...
                try:
                    self.feature_importances[index] = output['clf'].clf.theta_[1] - output['clf'].clf.theta_[0]
                    self.odds_ratio[index] = np.log(output['clf'].clf.theta_[1] / output['clf'].clf.theta_[0])
                    self.thetas[index] = output['clf'].clf.theta_
                except AttributeError:
                    try:
                        self.feature_importances[index] = output['clf'].clf.coef_
                    except AttributeError:
                        pass
...
class OnevsallClassifier(GenericClassifier):
    ...
    def load_data(self, features, X_threshold):
        ...
        for num, on_ids in enumerate(self.ids_by_masks):
            ...
            off_data = self.dataset.feature_table.get_feature_data(ids=off_ids, features=features).dropna()
...
class PairwiseClassifier(GenericClassifier):
    ...
    def load_data(self, features, X_threshold):
        ...
        self.comparisons = list(
            itertools.permutations(range(0, self.mask_num), 2))
