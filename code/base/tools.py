...
def region_vox_baserates(dataset, regions, remove_zero=True, method = 'mean'):
        ...
        Args:
            ...
            method: What type of calculation. 

            'mean' = just take the mean within a mask
            'conjunction' = likelihood any given voxel is active
        ...
        region_data = dataset.masker.mask(regions)

        levels = np.unique(region_data)[1:]
        ...
        labels = []
        for i in levels:
            data = dataset[np.where(region_data == i)[0]]
            ...
            if method == 'mean':
                ...
                base_rate = (data.sum(axis=0) > 0).sum() / (data.shape[1]*1.0)
                results.append(base_rate)
        ...
        results = pd.DataFrame([pd.Series(labels, dtype=object), pd.Series(results)]).T
...
def region_n_vox(dataset, regions, remove_zero=True):
    ...
    results = pd.DataFrame([pd.Series(range(1, len(results)+1)), pd.Series(results)]).T

    results.columns = ['region', 'n_vox']
    return results
...
def RGBToHTMLColor(rgb_tuple):
    ...
    rgb_tuple = tuple([n * 255 for n in rgb_tuple])
    hexcolor = '#%02x%02x%02x' % rgb_tuple
    # that's it! '%02x' means zero-padded, 2-digit hex values
    return hexcolor

def map_clusters(coarser, finer):
    cd = coarser.get_data().flatten()
    fd = finer.get_data().flatten()
    results = {}
    for i in np.unique(cd)[1:]:
        unique = np.unique(fd[cd == i].round().astype('int'))
        results[int(i)] = unique[unique.nonzero()]
    return results
