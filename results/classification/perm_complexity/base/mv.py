...
def bootstrap_mv_parallel(args):
    ...
    try:
        (X, y, classifier, scorer, fis, feature_names, method, reg), boot_n = args
        ...
        if method == 'sequential':
            ...
            for i in range(0, n_topics):
                ...
                for num, new_feat in enumerate(remaining):
                    ...
                    feature = feature_names[new_feat]
                    output = classify.classify(X_1, y, classifier=classifier, cross_val='4-Fold', scoring=scorer, output='summary')
                    test_results.append([output['score'], i, feature, reg, boot_n, new_feat])

                test_results = pd.DataFrame(test_results)

                winner = test_results[test_results.ix[:, 0] == test_results.ix[:, 0].max()]
...
def bootstrap_mv_full_parallel(args):
    ...
    try:
        ...
        for reg_i, reg_y_high in y_high.iterrows():
            reg_ix = ((y_low.iloc[reg_i, :] == True) & (reg_y_high == False)) == False
            ...
            if method == 'combinatorial':
                ...
                for i in range(0, n_topics):
                    ...
                    for num, new_feat in enumerate(remaining):
                        ...
                        X_1 = reg_X[:, try_comb]
                        feature = feature_names[new_feat]
                        output = classify.classify(X_1, reg_y.values, classifier=classifier, cross_val='4-Fold', scoring=scorer, output='summary')
                        test_results.append([output['score'], i, feature, reg_i, boot_n, new_feat])
